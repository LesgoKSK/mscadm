...
const source = path.join(here, "COMPREHENSIVE_RESEARCH_REPORT.md");
const output = path.join(here, "qa", "report_preview.html");
...
bodyRaw = bodyRaw.replace(/\[\[CUSTOM_FIGURE:([^|]+)\|([^\]]+)\]\]/g, (_m, name, cap) => {
  const file = name === "timeline" ? "research_timeline.png" : "ps_dfsc_gate_matrix.png";
  const uri = pathToFileURL(path.join(here, "assets", file)).href;
  const cls = name === "timeline" ? "timeline" : "ps-gates";
  return `\n<figure class="${cls}"><img src="${uri}" alt="${cap}"><figcaption>${cap}</figcaption></figure>\n`;
});
...
p { margin: 0 0 0.10in; text-align: justify; orphans: 3; widows: 3; }
...
<section class="cover">
  <div class="kicker">RESEARCH REPORT · EVIDENCE AUDIT · 2026</div>
  <h1>${title.replace("综合研究报告", "<br>综合研究报告")}</h1>
  <div class="subtitle">${subtitle}</div>
  <div class="rule"></div>
  <div class="meta">${meta}</div>
  <div class="badge">包含：正式复现 · 探索实验 · 内部确认 · exact SUC · 负结果与审计</div>
</section>
${bodyHtml}
</body></html>`;
