...
class GaussianDiffusion(nn.Module):
    ...
    def q_sample(
        ...
        noisy = noisy + extract(self.sqrt_one_minus_cumulative, timestep, clean.shape) * noise
        return noisy, noise
    ...
    def q_posterior(
        ...
        mean = mean + extract(self.posterior_coef2, timestep, noisy.shape) * noisy
    ...
    def sample_ddim(
        ...
        for index in reversed(range(len(selected))):
            ...
            alpha_previous = (
                self.cumulative[selected[index - 1]]
                if index > 0
