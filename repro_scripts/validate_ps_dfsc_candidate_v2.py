"""Validate final and resumable artifacts for one formal candidate."""
...
def main() -> None:
    ...
    state = resume["state"]
    if metadata.get("schema") != "ps_dfsc_epoch_resume_metadata_v2":
    ...
    if (
        float(metadata["beta"]) != args.beta
        or int(metadata["seed"]) != args.seed
        or int(metadata["epochs"]) != 50
        or int(metadata["warmup_epochs"]) != 20
        or int(metadata["batch_size"]) != 4
    ):
        raise ValueError("candidate resume configuration is invalid")
    if (
    ...
    evidence.update(
        {
            ...
            "resume_midpoint_refresh_completed": bool(
                resume["midpoint_refresh_completed"]
            ),
