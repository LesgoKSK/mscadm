...
def load_archives(root: Path) -> dict[str, np.lib.npyio.NpzFile]:
...
def scenario_panel(name: str, archive: np.lib.npyio.NpzFile, index: int, path: Path) -> None:
    ...
    axis.set(xlabel="Hour", ylabel="Normalized wind power", title=name, xlim=(1, 24), ylim=(0, 1))
    axis.legend(ncol=5, fontsize=8)
    figure.tight_layout(); figure.savefig(path, dpi=200); plt.close(figure)


def main() -> None:
    parser = argparse.ArgumentParser(description="Create paper-style Figures 4-7 without a GUI backend")
    ...
    root = Path(args.root); output = root / "figures"; output.mkdir(parents=True, exist_ok=True)
    data = load_archives(root / "scenarios")
    for name, archive in data.items():
        scenario_panel(name, archive, args.day_index, output / f"figure4_{name}.png")
    if not data:
        raise FileNotFoundError(f"No scenario archives under {root / 'scenarios'}")
    ...
    axes[1].set(xlabel="Nominal coverage", ylabel="ACE"); axes[1].axhline(0, color="black", linewidth=0.7)
    axes[0].legend(fontsize=7); figure.tight_layout(); figure.savefig(output / "figure5_intervals.png", dpi=200); plt.close(figure)
    figure, axis = plt.subplots(figsize=(7.2, 3.8))
    reference = next(iter(data.values()))["observations"].ravel()
    axis.hist(reference, bins=40, range=(0, 1), density=True, histtype="step", linewidth=2, color="black", label="observed")
    for name, archive in data.items():
    ...
    figure.tight_layout(); figure.savefig(output / "figure7_distribution.png", dpi=200); plt.close(figure)
    mscadm = next((archive for name, archive in data.items() if name.startswith("mscadm")), None)
    if mscadm is not None:
        for number, index in enumerate(np.flatnonzero(mscadm["zone"] == args.zone)[:3], 1):
            scenario_panel(
                f"MS-CADM Zone {args.zone}, day {number}",
                mscadm,
                int(index),
                output / f"figure6_zone{args.zone}_day{number}.png",
            )
