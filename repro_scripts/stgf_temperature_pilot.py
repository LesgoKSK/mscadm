...
from stgf_flow.sampling_calibrated import sample_stgf_calibrated
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibration-only residual temperature/anchor pilot"
    )
    parser.add_argument(
    ...
    temperatures = [0.45, 0.60, 0.75, 0.90, 1.00]
    anchors = [0.0, 0.5, 1.0]
    for anchor in anchors:
        for temperature in temperatures:
            name = f"{args.transform_mode}_a{anchor:.2f}_t{temperature:.2f}"
